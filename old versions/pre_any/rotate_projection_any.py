...
TRAIN_BASE = r"D:\med_data\biron\data1\train_fill"
...
# 旋转参数
# ============================================================
# 外层循环：DSA整体旋转角度 (0°, 10°, 20°, ..., 90°)
ROTATION_ANGLES = list(range(0, 121, 60))
# 内层循环：AP和LAT之间的夹角 (30°, 40°, 60°, 70°, 90°)
PROJECTION_ANGLES = [30, 50, 70, 90]
...
def save_comparison_image(ap_proj, lat_proj, mask_ap, mask_lat,
                          ap_overlay, lat_overlay, output_path, rot_angle, proj_angle):
    """保存综合对比图"""
    fig, axes = plt.subplots(2, 3, figsize=(15, 10))

    axes[0, 0].imshow(ap_proj, cmap='gray', interpolation='nearest', origin='lower')
    axes[0, 0].set_title(f'AP Projection (Rotation: {rot_angle}°, Angle: 0°)', fontsize=11, fontweight='bold')
    axes[0, 0].axis('off')
    ...
    axes[1, 0].set_title(f'LAT Projection (Rotation: {rot_angle}°, Angle: {proj_angle}°)', fontsize=11,
                         fontweight='bold')
    axes[1, 0].axis('off')
    ...
    plt.suptitle(f'DSA Projections - Rotation {rot_angle}°, Projection Angle {proj_angle}°',
...
def process_single_combination(case_num, attenuation_volume, mask_pruned,
    ...
    try:
        ...
        axes[0, 0].set_title(f'AP Projection (Rot: {rot_angle_deg}°, Angle: 0°)')
        axes[0, 0].axis('off')
        plt.colorbar(im1, ax=axes[0, 0], fraction=0.046)

        lat_display = lat_projection_resized.astype(np.float32)
        im2 = axes[0, 1].imshow(lat_display, cmap='gray')
        axes[0, 1].set_title(f'LAT Projection (Rot: {rot_angle_deg}°, Angle: {proj_angle_deg}°)')
        axes[0, 1].axis('off')
        ...
        plt.tight_layout()
        vis_path = os.path.join(output_dir, f'visualization_rot{rot_angle_deg}_proj{proj_angle_deg}deg.png')
        ...
        if generate_overlay:
            ...
            comparison_path = os.path.join(overlay_dir, f"comparison_rot{rot_angle_deg}_proj{proj_angle_deg}deg.png")
            save_comparison_image(
                ...
                comparison_path, rot_angle_deg, proj_angle_deg
            )
