...
class Encoder3D(nn.Module):
    """
    3D Encoder: 4个尺度，通道 [64, 128, 256, 512]
    """
    ...
    def forward(self, x):
        ...
        # 返回从深到浅: [512(8³), 256(16³), 128(32³), 64(64³)]
        return features[::-1]
...
class X2Shape(nn.Module):
    def __init__(self, img_size=128, in_chans=1, num_classes=2,
        ...
        self.cross_mambas = nn.ModuleList([
            CrossMamba(512),  # 最深尺度 (4³)
            CrossMamba(256),  # 次深尺度 (8³)
            CrossMamba(128),  # 次浅尺度 (16³)
            CrossMamba(64)    # 最浅尺度 (32³)
        ])

        # ========== 5. 上采样层 (3个) ==========
        self.upsample_layers = nn.ModuleList([
            nn.ConvTranspose3d(512, 256, kernel_size=2, stride=2),  # 4³ -> 8³
    ...
    def forward(self, x_ap, x_lat):
        ...
        # Step 5: U-Net 升采样路径
        current_feat = cross_outputs[0]  # (512, 4³)
...
if __name__ == "__main__":
    ...
    print("="*60)
    print("X2Shape 模型测试 (4尺度完整版，论文架构)")
    print("="*60)
    ...
    print(f"  Residual Blocks (3个): {res_params / 1e6:.2f}M")
