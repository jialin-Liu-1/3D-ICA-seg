...
class VolumetricBackprojection(nn.Module):
    ...
    volume_shape: 初始3D体积尺寸 (40, 40, 40) for img_size=160
    """
...
class X2Shape(nn.Module):
    def __init__(self, img_size=160, in_chans=1, num_classes=1,
        ...
        # 注意: dims_3d 是 [32, 64, 128, 256]
        # Encoder3D 输出从深到浅: [256@5³, 128@10³, 64@20³, 32@40³]
        # 所以 Cross-Mamba 的维度应该对应: 256, 128, 64, 32
    ...
    def forward(self, x_ap, x_lat):
        ...
        # Step 2: VBP
        f_ap_3d_base = self.vbp_ap(f_ap_2d)   # (B, 64, 40, 40, 40)
        f_lat_3d_base = self.vbp_lat(f_lat_2d)  # (B, 64, 40, 40, 40)

        vbp_jump_ap = f_ap_3d_base
        vbp_jump_lat = f_lat_3d_base

        # Step 3: 3D Encoder
        # 输出: [256@5³, 128@10³, 64@20³, 32@40³]
...
if __name__ == "__main__":
    ...
    print("="*60)
    print("X2Shape 模型测试 (适配通道减半 + 160尺寸)")
    print("="*60)

    # 您的配置
    model = X2Shape(
        img_size=256,           # 输入输出尺寸160
        in_chans=1,
        num_classes=1,          # 单通道输出
