...
class MambaVisionEncoder2D(nn.Module):
    ...
    2D 编码器: 输出 3 个尺度的特征图
    输入: 256x256
    输出尺度: 128x128, 64x64, 32x32
    通道: [dim0, dim1, dim2]
    """
    def __init__(self, in_chans=1, dims=[32, 64, 128], depths=[1, 1, 1]):
        super().__init__()
        self.dims = dims
    ...
    def forward(self, x):
        ...
        return features  # [dim0@128², dim1@64², dim2@32²]


# ============================================================
# 2. 体积反投影模块 (VBP) - 接收3个尺度输入
# ============================================================

class VolumetricBackprojection(nn.Module):
    """
    VBP: 接收多尺度 2D 特征 (3个尺度)，输出单一尺度 3D 特征
    volume_shape: 初始3D体积尺寸 (32, 32, 32) for img_size=256
    """
...
class Encoder3D(nn.Module):
    """
    3D Encoder: 3个尺度
    输入: (B, in_chans, 32, 32, 32)
    输出尺度: 4³, 8³, 16³
    通道: [channels[0], channels[1], channels[2]]
    """
    def __init__(self, in_chans, channels=[32, 64, 128]):
        super().__init__()
    ...
    def forward(self, x):
        ...
        # 返回从深到浅: [channels[2]@4³, channels[1]@8³, channels[0]@16³]
        return features[::-1]
...
# ============================================================
# 7. 完整的 X2Shape 模型 (3层U-Net结构)
# ============================================================

class X2Shape(nn.Module):
    def __init__(self, img_size=256, in_chans=1, num_classes=1,
                 dims_2d=[32, 64, 128], depths_2d=[1, 1, 1],
                 dims_3d=[32, 64, 128], vbp_output_channels=64):
        super().__init__()
        self.img_size = img_size
        self.num_levels = len(dims_3d)  # 3层

        # 体积尺寸: 32 -> 16 -> 8 -> 4 (for img_size=256)
        vol_sizes = [img_size // 8, img_size // 16, img_size // 32, img_size // 64]
        # vol_sizes = [32, 16, 8, 4]
        # VBP输出尺寸为32³
        ...
        # ========== 3. 两个独立的 3D Encoder (3个尺度) ==========
        self.encoder_3d_ap = Encoder3D(vbp_output_channels, dims_3d)
        self.encoder_3d_lat = Encoder3D(vbp_output_channels, dims_3d)

        # ========== 4. Cross-Mamba 模块 (3个) ==========
        # dims_3d = [32, 64, 128]
        # Encoder3D 输出从深到浅: [128@4³, 64@8³, 32@16³]
        self.cross_mambas = nn.ModuleList([
            CrossMamba(dims_3d[2]),  # 最深尺度: 128 (4³)
            CrossMamba(dims_3d[1]),  # 次深尺度: 64 (8³)
            CrossMamba(dims_3d[0])   # 最浅尺度: 32 (16³)
        ])

        # ========== 5. 上采样层 (2个) ==========
        self.upsample_layers = nn.ModuleList([
            nn.ConvTranspose3d(dims_3d[2], dims_3d[1], kernel_size=2, stride=2),  # 128 -> 64 (4³ -> 8³)
            nn.ConvTranspose3d(dims_3d[1], dims_3d[0], kernel_size=2, stride=2),  # 64 -> 32 (8³ -> 16³)
        ])
    ...
    def forward(self, x_ap, x_lat):
        ...
        # Step 2: VBP
        f_ap_3d_base = self.vbp_ap(f_ap_2d)   # (B, 64, 32, 32, 32)
        f_lat_3d_base = self.vbp_lat(f_lat_2d)  # (B, 64, 32, 32, 32)
...
if __name__ == "__main__":
    ...
    print("="*60)
    print("X2Shape 模型测试 (3层U-Net, 减少容量)")
    print("="*60)
