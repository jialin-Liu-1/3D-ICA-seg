...
# 导入模型
from model.x2shap_org import X2Shape
...
def preprocess_and_cache(data_dir, cache_root, img_size=128, train_ratio=0.8, seed=42):
    ...
    for case_name in tqdm(train_cases, desc="Training cases"):
        ...
        # ========== 单通道 mask 处理 ==========
        # mask: (128, 128, 128) 或 (128, 128, 128, 1) -> (1, 128, 128, 128)
        mask = mask.squeeze()  # 移除多余维度
        if mask.ndim == 3:
            mask = mask[None, :, :, :].astype(np.float32)  # (1, D, H, W)
        else:
...
class CarotidDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        mask = torch.from_numpy(data['mask'])  # (1, 128, 128, 128)
        ...
        with torch.no_grad():
            for batch in pbar:
                ...
                for i, case_name in enumerate(case_names):
                    # 转换回原始形状 (D, H, W) 单通道
                    pred_single = pred_mask[i, 0]  # (D, H, W)
                    gt_single = gt_mask[i, 0]  # (D, H, W)
