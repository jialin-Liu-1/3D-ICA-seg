...
def preprocess_and_cache(data_dir, cache_root, img_size=192, train_ratio=0.8, seed=42):
    ...
    for case_name in tqdm(train_cases, desc="Training cases"):
        ...
        # 确保形状正确
        # ap: (img_size, img_size) -> (1, img_size, img_size)
        ap = ap.squeeze()
        if ap.ndim == 2:
        ...
        # lat: (img_size, img_size) -> (1, img_size, img_size)
        lat = lat.squeeze()
        if lat.ndim == 2:
        ...
        # mask: (img_size, img_size, img_size) -> (1, img_size, img_size, img_size)
        mask = mask.squeeze()
        if mask.ndim == 3:
...
class CarotidDataset(Dataset):
    ...
    def __init__(self, cache_dir, case_list):
        ...
        # 从split_info.json读取图像尺寸
        with open(self.cache_dir / 'split_info.json', 'r') as f:
            split_info = json.load(f)
        self.img_size = split_info.get('img_size', 192)  # 默认192
    ...
    def __getitem__(self, idx):
        ...
        # 数据尺寸由预处理时的img_size决定
        ap = torch.from_numpy(data['ap'])  # (1, img_size, img_size)
        lat = torch.from_numpy(data['lat'])  # (1, img_size, img_size)
        mask = torch.from_numpy(data['mask'])  # (1, img_size, img_size, img_size)
    ...
    def test(self):
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(pbar):
                ...
                # 保存预测结果
                pred_mask = torch.sigmoid(output).cpu().numpy()  # (B, 1, D, H, W)
                gt_mask = mask.cpu().numpy()  # (B, 1, D, H, W)

                for i, case_name in enumerate(case_names):
                    # 转换回原始形状 (img_size, img_size, img_size) 单通道
                    pred_single = pred_mask[i, 0]  # (img_size, img_size, img_size)
                    gt_single = gt_mask[i, 0]  # (img_size, img_size, img_size)
...
def main():
    ...
    if existing_caches:
        ...
        if use_existing == 'y':
            ...
            cached_size = split_info.get('img_size', 256)
