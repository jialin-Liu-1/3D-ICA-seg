...
TRAIN_BASE = r"D:\med_data\biron\data1\test"
...
# 保存选项
# ============================================================
INVERT_PROJECTION = False  # True: 反转像素值后保存 (血管变暗，背景变亮)
...
def simulate_projection_gpu(attenuation_volume_gpu, axis):
    ...
    # 修复旋转伪影（背景消除功能）
    projection = fix_rotation_artifacts_gpu(projection)
...
def process_single_angle_gpu(case_num, attenuation_volume, original_dsa_volume, angle_idx, output_base,
    ...
    try:
        ...
        # 3. GPU投影计算
        # Y轴投影 (axis=1) - 正面投影 AP
        ap_projection_gpu = simulate_projection_gpu(rotated_atten_gpu, axis=1)
        # Z轴投影 (axis=2) - 侧面投影 LAT
        lat_projection_gpu = simulate_projection_gpu(rotated_atten_gpu, axis=2)

        # 4. 缩放投影（在GPU上）
        ap_projection_resized_gpu = resize_2d_image_gpu(ap_projection_gpu, TARGET_2D_SIZE)
        lat_projection_resized_gpu = resize_2d_image_gpu(lat_projection_gpu, TARGET_2D_SIZE)

        # 5. 转回CPU
        ap_projection_resized = cp.asnumpy(ap_projection_resized_gpu)
        lat_projection_resized = cp.asnumpy(lat_projection_resized_gpu)
        ...
        print(f"    AP投影: {ap_projection_resized.shape}, 范围 [{ap_projection_resized.min():.4f}, {ap_projection_resized.max():.4f}], 大小: {ap_size:.1f}KB")
        print(f"    LAT投影: {lat_projection_resized.shape}, 范围 [{lat_projection_resized.min():.4f}, {lat_projection_resized.max():.4f}], 大小: {lat_size:.1f}KB")
...
def test_mode_gpu():
    """测试模式（GPU加速版）"""
    print("\n" + "=" * 80)
    print("测试模式 - DSA扇形束蒙泰卡罗模拟 (GPU加速版)")
    print("=" * 80)
