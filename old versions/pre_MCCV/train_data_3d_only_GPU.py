...
# 处理选项
# ============================================================
NUM_COMPARE_IMAGES = 2  # 生成对比图的角度数量（前N个角度生成对比图）
...
def process_single_angle(case_num, original_volume, angle_idx, output_base, generate_compare=False):
    """
    处理单个角度的数据

    参数:
        case_num: 病例编号
        original_volume: 原始三维体积
...
def process_single_case_all_angles(case_num, output_base):
    ...
    print(f"\n病例{case_num}: 形状 {original_volume.shape}")
    ...
    for angle_idx in range(NUM_ANGLES):
        ...
        if process_single_angle(case_num, original_volume, angle_idx, output_base, generate_compare):
            success_count += 1
...
def test_mode():
    ...
    success = process_single_angle(TEST_CASE_NUM, original_volume, TEST_ANGLE_IDX, output_base, generate_compare=True)
