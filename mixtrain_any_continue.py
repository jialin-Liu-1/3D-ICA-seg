"""
X2Shape 完整训练脚本 - 单通道输出版（混合精度训练版）- 支持角度输入
模型导入: from model.x2shap_256_any import X2Shape
...
TRAIN_DATA_DIR = Path('/mnt/d/med_data/biron/data2/train_any')  # 训练集
TEST_DATA_DIR = Path('/mnt/d/med_data/biron/data2/test_any')  # 验证集 + 测试集
CACHE_ROOT = Path('/mnt/d/med_data/biron/cache')
MODEL_ROOT = Path('/mnt/d/med_data/biron/model_any')


# ============================================================
# 1. 数据预处理（缓存到 .npz 文件）- 自动适应尺寸 + 提取角度
# ============================================================
...
def parse_angle_from_folder(folder_name):
    """
    从文件夹名称中解析角度值
    文件夹命名格式: "病例号_角度"，如 "0_30"
    返回: 角度值 (int)，如果解析失败返回 90 (默认)
    """
...
def preprocess_and_cache(data_dir, cache_root, split_name, img_size=256):
    ...
    for case_dir in case_dirs:
        ...
        if ap_path.exists() and lat_path.exists() and mask_path.exists():
            ...
            # 解析角度
            angle = parse_angle_from_folder(case_name)
            case_angles[case_name] = angle
        else:
    ...
    split_info = {
        ...
        'cases': valid_cases,
        'case_angles': case_angles,  # 保存每个病例的角度
    ...
    for case_name in tqdm(valid_cases, desc=f"{split_name} cases"):
        ...
        np.savez(
            ...
            lat=lat,
            mask=mask,
            angle=np.array(angle, dtype=np.int32)  # 保存角度
        )
    ...
    print(f"实际图像尺寸: {actual_img_size}×{actual_img_size} (2D), {actual_img_size}×{actual_img_size}×{actual_img_size} (3D)")
    print(f"缓存目录: {split_dir}")
...
def preprocess_all_data(train_dir, test_dir, cache_root, img_size=256):
    ...
    # 预处理训练集
    train_cache_dir, train_cases, train_angles = preprocess_and_cache(
        data_dir=train_dir,
    ...
    # 预处理验证集（test1）
    val_cache_dir, val_cases, val_angles = preprocess_and_cache(
        data_dir=test_dir,
        cache_root=cache_root,
        split_name='val_test',
        img_size=img_size
...
# ============================================================
# 2. 数据集类（流式读取）- 自动适应尺寸 + 返回角度
# ============================================================
class CarotidDataset(Dataset):
    """从缓存文件读取数据 - 自动适应尺寸，返回角度信息"""
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            'x_lat': lat,
            'mask': mask,
            'angle': torch.tensor(angle, dtype=torch.long),  # 返回角度张量
            'case_name': self.case_list[idx]
        }
...
class Trainer:
    def __init__(self, model, train_loader, val_loader, test_loader,
                 ...
                 save_dir=None, patience=6, min_delta=0.0003,
                 lr_cosine_cycles=1, min_lr=1e-7,
                 lr_patience=3, warmup_epochs=0,
                 start_reduce_epoch=10,
                 pretrained_path=None):  # 新增参数：预训练模型路径
        """
        参数:
            patience: 早停耐心值 (连续6轮不改进后停止训练)
            lr_patience: 学习率衰减耐心值 (连续3轮不改进后触发余弦退火)
            lr_cosine_cycles: 每次余弦退火的周期数 (1表示半个余弦周期)
            min_lr: 最小学习率 (余弦退火的目标值)
            warmup_epochs: 预热轮数 (设为0表示不启用)
            start_reduce_epoch: 从第几轮开始启用早停和学习率衰减
            pretrained_path: 预训练模型路径，用于继续训练
        """
        self.model = model
        ...
        self.pretrained_path = pretrained_path  # 保存预训练路径
    ...
    def load_pretrained_model(self, pretrained_path):
        ...
        try:
            ...
            if 'history' in checkpoint:
                ...
                # 检查history的类型，如果是字典则转换为列表
                if isinstance(history, dict):
                    for key in ['train_loss', 'val_loss', 'train_dice', 'val_dice', 'lr']:
                        if key in history:
                            # 如果history[key]是张量，转换为列表
                            if torch.is_tensor(history[key]):
                                self.history[key] = history[key].tolist()
                            else:
                                self.history[key] = list(history[key])
                print(f"✓ 训练历史恢复成功 (已训练 {len(self.history['train_loss'])} 轮)")

            # 恢复最佳指标（如果有）
            if 'best_dice' in checkpoint:
                self.best_dice = float(checkpoint['best_dice'])
                print(f"✓ 最佳Dice恢复: {self.best_dice:.4f}")

            if 'best_epoch' in checkpoint:
                self.best_epoch = checkpoint['best_epoch']
                print(f"✓ 最佳轮次恢复: {self.best_epoch + 1}")
            ...
            print(f"\n✓ 预训练模型加载完成！将从第 {self.start_epoch + 1} 轮继续训练")
    ...
    def cosine_annealing_lr(self):
        """
        余弦退火学习率衰减
        每次触发时，在当前学习率的基础上进行一个余弦周期的衰减
        """
        current_lr = self.optimizer.param_groups[0]['lr']

        # 如果当前学习率已经接近最小学习率，不再衰减
        if current_lr <= self.min_lr * 1.1:
            print(f"\n  → 当前学习率 {current_lr:.2e} 已达到或接近最小学习率 {self.min_lr:.2e}，停止衰减")
            return False

        # 余弦退火：从当前学习率衰减到 min_lr，然后再回到当前学习率（单周期）
        # 这里我们使用半个余弦周期，从当前学习率平滑衰减到 min_lr
        # 但因为我们是在每个epoch触发一次，所以我们可以设置多个step

        # 计算新的学习率：使用余弦退火公式
        # new_lr = min_lr + 0.5 * (current_lr - min_lr) * (1 + cos(pi * t / T))
        # 其中 t 是当前步数，T 是总步数

        # 使用多个步骤来完成一次完整的余弦退火
        self.cosine_epoch_counter += 1

        # 每次衰减时，计算当前步骤在余弦周期中的位置
        # T 是总周期长度（epoch数），这里设置为10
        t = self.cosine_epoch_counter % self.cosine_total_epochs
        T = self.cosine_total_epochs

        # 余弦退火公式（半个周期从 high 到 low）
    ...
    def train_epoch(self, epoch):
        ...
        for batch in pbar:
            ...
            angle = batch['angle'].to(self.device)  # 获取角度

            self.optimizer.zero_grad()

            with autocast('cuda'):
                # 将角度传递给模型
                output = self.model(x_ap, x_lat, angle=angle)
    ...
    def train(self):
        ...
        print(f"早停耐心值 (连续不改进): {self.patience} 轮")
        print(f"学习率衰减耐心值 (连续不改进): {self.lr_patience} 轮")
        ...
        if hasattr(self, 'start_epoch') and self.start_epoch > 0:
            print(f"继续训练: 从第 {self.start_epoch + 1} 轮开始 (已训练 {self.start_epoch} 轮)")
        print("=" * 60)
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            if val_metrics['dice'] > self.best_dice:
                ...
                torch.save({
                    ...
                    'best_dice': torch.tensor(self.best_dice),  # 转为张量
                    'best_epoch': epoch,
                    'metrics': {k: torch.tensor(v) for k, v in val_metrics.items()},  # 转为张量
                    'history': self.history  # 保存完整历史
            ...
            if epoch >= self.start_reduce_epoch:
                ...
                if val_metrics['dice'] > self.best_dice - self.min_delta:
                    ...
                    if self.lr_patience_counter >= self.lr_patience:
                        print(f"\n  ⚠️ 连续 {self.lr_patience_counter} 次未改进！触发余弦退火学习率衰减...")
                    ...
                    if self.patience_counter >= self.patience:
                        print(f"\n  ⚠️ 连续 {self.patience_counter} 次未改进！触发早停...")
                        print(f"最佳轮次: {self.best_epoch + 1} (Dice: {self.best_dice:.4f})")
                        break

        history = {
            'train_loss': torch.tensor(self.history['train_loss']),
            'val_loss': torch.tensor(self.history['val_loss']),
            'train_dice': torch.tensor(self.history['train_dice']),
            'val_dice': torch.tensor(self.history['val_dice']),
            'lr': torch.tensor(self.history['lr'])
        }
    ...
    def test(self):
        ...
        if 'best_dice' in checkpoint:
            ...
            print(f"加载最佳模型 (Dice: {best_dice:.4f})")
        else:
            print(f"加载最佳模型")
...
def main():
    ...
    # 添加预训练模型路径
    PRETRAINED_MODEL_PATH = "/mnt/d/med_data/biron/model_any/20260720_776(1)/best_model.pth"

    print("=" * 60)
    print("X2Shape 颈总动脉分割训练 (单通道输出) - 支持角度输入")
    print("=" * 60)
    ...
    if Path(PRETRAINED_MODEL_PATH).exists():
        ...
        print(f"⚠️ 警告: 预训练模型不存在: {PRETRAINED_MODEL_PATH}")
        print("将从头开始训练...")
        ...
        if use_existing == 'y':
            ...
            if latest_cache is not None:
                ...
                if cached_size != IMG_SIZE:
                    ...
                    train_angles = total_info.get('train_angles', {})
                    val_angles = total_info.get('val_angles', {})
                    print(f"\n使用缓存: {latest_cache}")
                    print(f"训练集缓存: {train_cache_dir}")
                    print(f"验证集缓存: {val_cache_dir}")
                    print(f"训练集: {len(train_cases)} 个病例")
                    print(f"验证集: {len(val_cases)} 个病例")
                    print(f"训练集角度分布: {set(train_angles.values()) if train_angles else '未知'}")
            else:
    ...
    test_dataset = CarotidDataset(val_cache_dir, val_cases)  # 验证集和测试集使用相同数据
    ...
    # ========== 创建 Trainer 使用新参数 ==========
    # 如果预训练模型存在，则使用它
    pretrained_path = PRETRAINED_MODEL_PATH if Path(PRETRAINED_MODEL_PATH).exists() else None
    ...
    trainer = Trainer(
        ...
        patience=8,  # 早停: 连续12次不改进后停止
        min_delta=0.001,  # 最小改进阈值
        lr_cosine_cycles=1,  # 余弦退火周期数
        min_lr=1e-6,  # 最小学习率 (余弦退火目标值)
        lr_patience=4,  # 学习率衰减: 连续3次不改进后触发余弦退火
        warmup_epochs=0,  # 不启用预热
        start_reduce_epoch=4,  # 从第10轮开始启用
        pretrained_path=pretrained_path  # 传入预训练模型路径
    )
