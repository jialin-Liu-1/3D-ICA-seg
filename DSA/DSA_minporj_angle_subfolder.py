...
def process_paired_dsa_images(input_folder, output_folder, time_fraction=0.5,
    ...
    for pair_info in tqdm(paired_images, desc="处理配对图像"):
        ...
        try:
            ...
            logger.info(f"View1角度: {angle1}")
            logger.info(f"View2角度: {angle2}")
            ...
            # 确定哪个角度小（ap），哪个角度大（lat）
            # 注意：这里使用的是原始角度值（可能是负数），比较时使用原始值
            if angle1 is not None and angle2 is not None:
                # 比较角度大小
                if angle1 < angle2:
                    ap_angle = angle1
                    lat_angle = angle2
                    ap_image = min_projection1
                    lat_image = min_projection2
                    ap_angle_str = format_angle(angle1)
                    lat_angle_str = format_angle(angle2)
                else:
                    ap_angle = angle2
                    lat_angle = angle1
                    ap_image = min_projection2
                    lat_image = min_projection1
                    ap_angle_str = format_angle(angle2)
                    lat_angle_str = format_angle(angle1)
                ...
                logger.info(f"AP角度: {ap_angle} ({ap_angle_str})")
                logger.info(f"LAT角度: {lat_angle} ({lat_angle_str})")

                # 保存AP图像
                logger.info(f"保存 AP 图像...")
                if save_as_nifti(ap_image, ap_path, np.eye(4), invert_pixels):
                    processed_pairs += 1
                else:
                    skipped_pairs += 1
                    continue
                ...
                if save_as_nifti(lat_image, lat_path, np.eye(4), invert_pixels):
                    processed_pairs += 1
                else:
                    skipped_pairs += 1
            else:
                # 如果角度提取失败，使用view1/view2命名
                logger.warning(f"角度提取失败，使用view1/view2命名")

                # 生成角度字符串（如果提取失败）
                angle1_str = format_angle(angle1)
                angle2_str = format_angle(angle2)

                output_filename1 = f"view1_{angle1_str}.nii.gz"
                output_filename2 = f"view2_{angle2_str}.nii.gz"

                output_path1 = os.path.join(pair_folder, output_filename1)
                output_path2 = os.path.join(pair_folder, output_filename2)

                logger.info(f"保存 View1...")
                if save_as_nifti(min_projection1, output_path1, np.eye(4), invert_pixels):
                    processed_pairs += 1
                else:
                    skipped_pairs += 1
                    continue
                ...
                if save_as_nifti(min_projection2, output_path2, np.eye(4), invert_pixels):
                    processed_pairs += 1
                else:
                    skipped_pairs += 1
